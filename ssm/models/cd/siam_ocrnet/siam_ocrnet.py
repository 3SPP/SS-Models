...
from ssm.models.utils import ConvBNReLU, constant_init, normal_init
...
class SiamOCRNet(nn.Layer):
    ...
    def __init__(self,
        ...
        if siam is False and cat is True:
            self.backbone.reset_layer1(self.backbone.in_channels * 2)
        self.backbone_indices = backbone_indices
        if siam is True and cat is True:
            in_channels = [(2 * self.backbone.feat_channels[i]) for i in backbone_indices]
        else:
    ...
    def forward(self, x1, x2):
        if self.siam:
            ...
            if self.cat:
                feats = [paddle.concat([feats1[i], feats2[i]], axis=1) for i in self.backbone_indices]
            else:
