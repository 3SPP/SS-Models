...
def slide_inference(model, im1, im2, crop_size, stride):
    """
    Infer by sliding window.
    Args:
        model (paddle.nn.Layer): model to get logits of image.
        im1 (Tensor): the input image.
        im2 (Tensor): the input image.
    ...
    for r in range(rows):
        for c in range(cols):
            ...
            h1 = max(h2 - h_crop, 0)
            w1 = max(w2 - w_crop, 0)
            im1_crop = im1[:, :, h1:h2, w1:w2]
            if im2 is not None:
                im2_crop = im2[:, :, h1:h2, w1:w2]
                logits = model(im1_crop, im2_crop)
            else:
                logits = model(im1_crop)
...
def inference(model,
    ...
    if not is_slide:
        ...
        logit = slide_inference(model, im1, im2, crop_size=crop_size, stride=stride)
    if hasattr(model, 'data_format') and model.data_format == 'NHWC':
...
def aug_inference(model,
    ...
    for scale in scales:
        ...
        for flip in flip_comb:
            im1_flip = tensor_flip(im1, flip)
            im2_flip = tensor_flip(im2, flip) if im2 is not None else None
            logit = inference(
                model,
                im1_flip,
                im2_flip,
