...
def train(model,
    ...
    Args:
        ...
        train_dataset (ssm.datasets.RSDataset): Used to read and process training datasets.
        val_dataset (ssm.datasets.RSDataset, optional): Used to read and process validation datasets.
        ...
        for data in loader:
            ...
            if fp16:
                with paddle.amp.auto_cast(
                    ...
                    if image2 is None:
                        if nranks > 1:
                            logits_list = ddp_model(image1)
                        else:
                            logits_list = model(image1)
                    else:
                        if nranks > 1:
                            logits_list = ddp_model(image1, image2)
                        else:
                            logits_list = model(image1, image2)
